...
controller = ManipulatorMRACRBF(
    ...
    Lambda=2.,  #lambda*KD is weight on position error qdes - q
    Gamma=100,  #don't love having this so high, its kind of like the itegrator
    KD=40,  #weight on velocity error, too high causes chattering
...
with viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        if data.time > 0:
            start = time.time()
            J = kinematics.getFullGeometricJacobian(q0, q1, q2, "WORLD",
                                                    "JOINT2_TOP")
            print(f"Time to get Jacobian: {time.time() - start}")
