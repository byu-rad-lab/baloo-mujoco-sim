...
controller = ManipulatorMRACRBF(
    num_gen_coords=6,
    numberOfRBFCenters=
    30,  #number doesn't seem to matter too much for performance, but more smooths out the control signal to a point.
    RBFmins=np.array([-100] * 6 * 4) *
    1.0,  #TODO: stopping parameter adjustment somehow? saturate? sat() function doesn't seem to do much.
    RBFmaxes=np.array([100] * 6 * 4) * 1.0,
    zeta=np.array([1] * 6),
    ...
    KD=50,  #weight on velocity error
...
with viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        start = time.time()
        tau, s, theta_hat, tau_ff, tau_pd = controller.solve_for_next_u(
    ...
    for i in range(6):
        ...
        axs[i].grid()
        axs[i].legend()
    print(f"CENTERS: {controller.centers}")
    plt.show()
